...
public class CountTopology {
    ...
    private static final Serde<Book> booksSerde = new SecuredSpecificAvroSerde<>(); // encoded
    private static final Serde<Order> ordersSerde = new SecuredSpecificAvroSerde<>(); // encoded
    private static final Serde<Count> countsSerde = new SpecificAvroSerde<>(); // plaintext
    ...
    public KStream<String, Count> count(StreamsBuilder streamsBuilder, Environment env) {
        ...
        final KTable<String, Count> countsTable = ordersStream.groupBy((m, n) -> n.getBookId())
                .aggregate(() -> 0, (bookId, order, total) -> order.getQuantity() + total)
